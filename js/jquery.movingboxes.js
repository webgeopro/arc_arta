...
;(function($){
	$.movingBoxes = function(el, options){
		...
		base.init = function(){
			...
			// Определяем аттрибут name текущего img (для получения текста из связанного div-а)
            var curImage = '';

			// Set up click on left/right arrows
			base.$left = base.$wrap.find('.mb-right').click(function(){ //mb-left
				curImage = parseInt(base.$wrap.find('.mb-panel.current img').attr('name')) - 1;
                base.goBack();
                setProductText(curImage, base.options.targetDiv, base.options.sliderID);
				return false;
			});
			base.$right = base.$wrap.find('.mb-left').click(function(){ //mb-right
                curImage = parseInt(base.$wrap.find('.mb-panel.current img').attr('name')) + 1;
                base.goForward();
                setProductText(curImage, base.options.targetDiv, base.options.sliderID);//base.options.targetDiv
				return false;
			});
    ...
    $.fn.setProductText = function(i){
        if (typeof i == "undefined") i = 0;
        var curImage = parseInt(base.$wrap.find('.mb-panel.current img').attr('name')) + 1;
        var txt = $("#txt_div_"+curImage).html();
        $("#"+base.$wrap.targetDiv).html(txt);
	};

})(jQuery);
/**
 * Вставка текста из связанного div-а в targetDiv (Для каждого слайдера свой)
 * @param obID
 * @param targetDiv
 */

function setProductText(obID, targetDiv, sliderID)
{
    //var curImage = parseInt(base.$wrap.find('.mb-panel.current img').attr('name'))
    var txt = $("#txt_div_"+sliderID+"_"+obID).html();
    //alert("#txt_div_"+sliderID+"_"+obID+"\n"+txt);
    //alert("txt="+txt);
    $("#"+targetDiv).html(txt);
}